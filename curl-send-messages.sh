...
while true ; do 
  ...
  curl -X POST -H 'Content-Type: application/json' --data '{"text":"Sending Message to test DR POC","attachments":[{"title":"'"Site: ${SITE}"'","text":"'"DR message ${NUMBER}"'"}]}' ${HOOK1}
  curl -X POST -H 'Content-Type: application/json' --data '{"text":"Sending Message to test DR POC","attachments":[{"title":"'"Site: ${SITE}"'","text":"'"DR message ${NUMBER}"'"}]}' ${HOOK2}
done
